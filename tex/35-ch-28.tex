...
\begin{quote}
\textbf{Intelligence is the ability of a system to shape and exploit regions of MRMW---worldlines, bundles, and even rules---under resource constraints, while maintaining control over what happens in and around it.}
\end{quote}
...
\begin{itemize}
  \item define agents as embedded RMG subgraphs with policies that steer MRMW,
...
\begin{itemize}
  \item spin up new RMG universes (new rule sets),
...
- Rollouts here \emph{are} real RMG universes in CΩMPUTER.
...
\begin{itemize}
  \item collectives can operate on shared, explicit RMGs of their own behavior,
...
\begin{enumerate}
  \item \textbf{Make histories first-class.} Training, inference, and interaction are all RMGs with explicit provenance, not opaque blobs of log files.
...
\begin{itemize}
  \item \textbf{A universal substrate} (RMG + DPO) in which brains, models, institutions, and environments are all first-class and composable.
