...
\begin{enumerate}
  \item \textbf{Many actual worlds.} Counterfactual runs are not hypothetical scribbles;
        they are first-class RMG universes. You can create, alter, and destroy vast families of them.
...
\begin{itemize}
  \item $\mathcal{W}$ be the set of worlds we care about: each $w \in \mathcal{W}$ is
        a coarse-grained RMG universe (a particular history or bundle, up to some equivalence).
...
\begin{itemize}
  ...
  \item Capacity to experience valence-like states (where valence is itself a functional over the RMG, not a mystical qualia tag);
...
- static analyses over candidate RMG + rule specs,
