\chapter*{Note to the Reader: The Computational Cosmos}
\addcontentsline{toc}{chapter}{Note to the Reader: The Computational Cosmos}
...
\begin{itemize}
    \item \textbf{A single shared-memory race can yield $10^{12}$ possible interleavings per second} on a modern multicore CPU. Most never appear in testing; all are permitted by the model.
    \item \textbf{91\% of production outages} in large distributed systems are traced to inconsistent state, emergent nondeterminism, or unobservable failure modes.
    \item \textbf{Debugging consumes 50--60\% of engineering effort} industry-wide. We spend more time reconstructing what happened than designing what should happen.
...
I didn't realize it then, but that twisted broccoli sparked a fifteen-year-long quiet fascination with graphs. That broccoli taught me about \textbf{Recursive Meta-Graphs (RMGs)}. It's also why you're reading this book right now.
...
\begin{enumerate}
    \item \textbf{Graphs within Graphs (RMGs)}: State is a \textbf{Recursive Meta-Graph} that allows for fractal, hierarchical representation of any structured data.
...
\begin{itemize}
  \item \textbf{Parts I--III} build the core ontology and the ``physics'' of \COMPUTER{}: Recursive Meta-Graphs (RMGs), double-pushout rewrite, MRMW (the phase space of all computations), curvature, superposition as rewrite bundles, and measurement as minimal path collapse.
  ...
  \item \textbf{Part VI} is the jump: treating our physical universe as just another RMG (C$\Omega$SMOS), and intelligence and ethics as geometry problems in MRMW.
\end{itemize}
