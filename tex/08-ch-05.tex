...
This set is the \textbf{local slice} of Rulial Space. It is the subgraph of all reachable WARP states exactly one tick away from the current worldline, bounded strictly by your specific rule system. We call this the \textbf{Kairos plane}—the field of immediate legal options.
...
\subsection{Kairos — The Time Cone}
...
\begin{tikzpicture}[scale=1.2, x={(1cm,0cm)}, y={(0.5cm,0.5cm)}, z={(0cm,1cm)}]
    % Draw Axes
    \draw[->, thick, gray] (0,0,0) -- (5,0,0) node[anchor=west, xshift=4pt, outer sep=2pt]{Chronos (Time)};
    \draw[->, thick, gray] (0,0,0) -- (0,4,0) node[anchor=west, xshift=4pt, yshift=2pt, outer sep=2pt]{Kairos (Opportunity)};
    \draw[->, thick, gray] (0,0,0) -- (0,0,4) node[anchor=west, xshift=4pt, outer sep=2pt]{Aios (Structure)};
...
\section{The Time Cone: A Local Lens on Rulial Space}
\label{the-time-cube-a-local-lens-on-rulial-space}

You can visualize the immediate future not just as a list of options, but geometrically as a \textbf{Time Cone}—a \textbf{cone of possible futures}.
...
The Time Cone is the lens through which the \textbf{Observer} sees where the Scheduler \textit{could} go next. It reveals that you cannot go everywhere; you can only reach the nearby computational futures allowed by the geometry of the graph.
...
To make this concrete, let us abandon abstract graphs and use a tangible example we will return to throughout this book: the \textbf{Life-Block}. Imagine a tiny, 3x3 binary grid universe with two simple rules:
...
A WARP graph stores state. A DPO rewrite transforms state. Therefore, it follows that \textbf{WARP equals storage}, and \textbf{DPO equals computation}. But when you look closer, the distinction vanishes.

\textbf{Storage is simply frozen computation. Computation is simply storage in motion.}

There is no longer a conceptual split between code and data. The Life-Block grid \emph{is} data, but it is also the \emph{program} that generates the next grid.
...
For a state $s \in \mathcal{S}$, the \emph{local slice} (or \emph{Time Cone}) is
\[
...
% --- LEFT: UNIT COST METRIC ----------------------------------------

% Title
\node at (1.8, 3.2) {\textbf{Unit-Cost Distance}};

% Nodes
\node[circle, draw, fill=gray!10] (A) at (0,0) {$s$};
\node[circle, draw, fill=gray!10] (B) at (2,1) {};
\node[circle, draw, fill=gray!10] (C) at (4,0) {$t$};
...
% Distance label
\node at (2,-0.8) {$d(s,t) = 2$};


% --- RIGHT: WEIGHTED COST METRIC ----------------------------------

% Title
\node at (8.2, 3.2) {\textbf{Weighted Distance via $\mathcal{L}$}};

% Nodes
\node[circle, draw, fill=gray!10] (A2) at (6,0) {$s$};
\node[circle, draw, fill=gray!10] (B2) at (8,1) {};
\node[circle, draw, fill=gray!10] (C2) at (10,0) {$t$};

% Edges with Lagrangian weights
\draw[->, thick] (A2) -- node[above] {$\mathcal{L}_1$} (B2);
\draw[->, thick] (B2) -- node[above] {$\mathcal{L}_2$} (C2);
...
Let $\mathcal{C}$ be an adhesive category (e.g., a category of graphs or hypergraphs) with pushouts and pullbacks along monomorphisms. A \emph{DPO rule} is a span of monomorphisms
\[
...
\begin{remark}
If this conjecture holds even in restricted classes of WARP graphs, it opens the door to treating long-running computations as trajectories in an emergent geometric space, with optimization, scheduling, and verification all recast as geometric problems about geodesics, curvature, and volume.
\end{remark}
...
\item \textbf{Part III: The Physics of C{\Large$\Omega$}MPUTER}  
      \begin{itemize}
         ...
         \item \textbf{Chapter~\ref{chapter-10-local-np-collapse} (Local NP Collapse)}\,:  
         ...
         \item \textbf{Chapter~\ref{chapter-11-rewrite-bundles} (Rewrite Bundles and Superposition)}\,:  
         ...
         \item \textbf{Chapter~\ref{chapter-12-interference} (Interference and Collapse)}\,:  
...
\item \textbf{Appendices}  
      \begin{itemize}
          \item The categorical DPO formalism is expanded into a full algebraic semantics for 
                WARP graph rewriting, including adhesive-category properties, functorial semantics, 
...
We have established the vocabulary of the new world. We have seen the lens (Time Cone), the space (Aios), the actual path (Chronos), and the geometry (Rulial Distance). We can finally answer the question: \textbf{``What is execution, really?''}
...
$\mathcal{R}$ & The DPO rule algebra governing legal rewrites in the WARP graph universe. \\[2pt]
