...
\section{Why Counterfactual Execution Is Safe in WARP+DPO}
...
\begin{tikzpicture}[node distance=2cm]
    % Nodes
    \node (start) [rectangle, draw, rounded corners, fill=white, text width=3cm, align=center] {Current WARP State};
...
\section{The Time Cone as Input}

We previously discussed the Time Cone (Chapter 5) as a theoretical construct—a visualization of local history and future possibility. For the CFEE, the Time Cone is not a diagram; it is the input data structure.

The ``Bundle'' is the entry point. The ``Light Cone'' is the horizon of the search depth. The ``Neighborhoods'' define the structure of the search space. When the CFEE runs, it is literally reading the geometry of the Time Cone to make decisions. This is what makes WARP+DPO a computational substrate rather than just a model: the geometry *is* the logic.
...
\begin{itemize}
    \item The **Time Cone** represents the local branching factor of the graph.
