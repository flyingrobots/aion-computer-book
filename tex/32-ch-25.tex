...
\emph{statistics and invariants of rewrite} on a particular class of RMGs.
The universe isn’t running on a computer; the universe is what it feels like
to be a very specific CΩMPUTER instance.

\subsubsection*{25.1  Substrate First: RMG + DPO as "World"}

We start from the ontology built in Parts I–III:

\begin{itemize}
  \item A \textbf{Recursive Meta-Graph} (RMG) is the underlying state object.
...
\begin{enumerate}
  \item There exists a \textbf{coarse-graining} $C$ from the raw RMG state $G$
...
indistinguishable from "physics," whether or not they know RMGs or DPO exist.

The rest of the chapter is just spelling out those bullets in RMG language.
...
\begin{itemize}
  \item A \textbf{microstate} as a full RMG $G$.
...
\subsubsection*{25.7  Landauer in RMG Clothing}
...
\begin{tabular}{ll}
\toprule
\textbf{Physics Concept} & \textbf{CΩMPUTER / RMG Concept} \\
\midrule
State of the world & RMG $G$ (possibly with layered structure) \\
...
RMG + rule set deserves to be taken seriously as a "physical" universe.
...
\begin{itemize}
  ...
  \item We grounded standard physical notions (conservation, symmetry, light cones,
        entropy, time’s arrow) directly in RMG + DPO.
...
how do we \emph{compile} it into an RMG + DPO rule set such that the mapping
