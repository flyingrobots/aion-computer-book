\chapter{Worldlines: Execution as Geodesics}\label{chapter-6-worldlines-execution-as-geodesics}

\section{\texorpdfstring{\textbf{What it means for a computation to
...
That chain of states, from tick 0 → tick N, is your \textbf{worldline}.
...
\textbf{State → rewrite → state → rewrite → state}

A worldline is the \emph{actual} history.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{\textbf{6.2 --- Why CΩMPUTER's Worldlines Are
Deterministic}}{6.2 --- Why CΩMPUTER's Worldlines Are Deterministic}}\label{why-cux3c9mputers-worldlines-are-deterministic}
...
One tick → one rewrite → one next universe. No randomness. No
...
\section{\texorpdfstring{\textbf{Worldlines and Lambda Calculus
Reduction
