...
\section{6.1 --- What Is a Worldline?}
\label{6.1-what-is-a-worldline}
...
actually happens. In RMG space, execution is nothing but an unbroken chain of graph states:
...
\midrule
State & RMG $S$ & Heap / object graph at a tick \\
...
\section{6.2 --- Why Worldlines Are Deterministic}
...
\begin{enumerate}
    ...
    All matches are expressed as tuples of node IDs, edge IDs, and port bindings. Because the RMG
...
This hierarchy ensures that the transition $S \rightarrow S{+}1$ is both deterministic and
\textbf{canonical}. Replay the same input, you get the same universe. Byte-for-byte, edge-for-edge.

This property is the bedrock on which everything else stands. (For the formal specification, see Appendix~\ref{appendix:scheduler-spec}).

\begin{center}
\rule{0.5\linewidth}{0.5pt}
\end{center}

\section{6.3 --- Worldline Sharpness: Sensitivity to Initial Conditions}
...
\section{6.4 --- Geodesics: The ``Straight Lines'' of Computation}
\label{6.4-geodesics}
...
We define the \textbf{local friction tensor} $\Phi(S, m)$ as:
...
We then define the \textbf{Refactor Difficulty} tensor:
...
\section{6.5 --- Collapse: Choosing One Future}
\label{6.5-collapse}

The Time Cube presents a cloud of potential futures; the worldline requires a single committed one.
...
\section{6.6 --- Worldlines Are Debugging}
...
RMG-based debugging gives you the \emph{path} itself. You can examine where the worldline bent,
...
\paragraph{The \texttt{git bisect} Analogy.}
If a worldline is a commit history in the universe, then debugging it is a \emph{binary search on
reality}. Worldline diffing is the geometric form of \texttt{git bisect}: a deterministic search for
...
Diffing worldlines is not textual---it is geometric. Imagine overlaying the
...
any structural difference---a missing edge, an extra allocation, a mutated
subgraph---begins to glow.
...
\section{6.7 --- Worldlines Are Optimization}
...
redex collapse. But the analogy breaks quickly: RMG reductions operate in a metric space, support
...
The difference is that RMG space is curved, structured, and globally coherent.

\begin{center}
\rule{0.5\linewidth}{0.5pt}
\end{center}

\section{6.8 --- Transition: From Worldlines to Neighborhoods}
...
\label{appendix:scheduler-spec}

\noindent\textit{This appendix is not required to understand Chapter 6, but provides the formal machinery for those implementing the runtime.}

In this appendix we make precise what was stated informally in
...
\begin{itemize}
    \item $\mathcal{S}$ be the set of all valid RMG universe states.
...
\section{1. Stable Identity Invariant}

Every node and edge in an RMG must carry a \textbf{stable, opaque identity}
...
\section{2. Deterministic Match Enumeration}
...
\section{3. Lexicographic Rule Ordering Must Be Canonical}
...
\section{4. Conflict Resolution Must Be Purely Structural}
...
\begin{itemize}
    ...
    \item The greedy leftmost-priority rule must be used exactly as
          defined in Appendix~\ref{appendix:scheduler-spec}.
...
\section{5. Canonical Embedding Hash Must Be Deterministic}
...
\section{6. DPO Construction Must Be Canonical}
...
\begin{itemize}
    \item stored explicitly in the RMG, or
