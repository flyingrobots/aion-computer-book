...
\begin{center}
\emph{Given RMG + DPO, here’s how physics shows up as invariants and statistics.}
\end{center}

Now we flip it:

\begin{center}
\emph{Given a physical theory, how do we compile it into RMG + DPO so it
...
\[
  ...
  (\text{RMG } G_\mathcal{T}, \text{ rule set } \mathcal{R}_\mathcal{T})
\]
...
- the symmetries and conservation laws of $\mathcal{T}$ are RMG symmetries and invariants,
...
\begin{itemize}
  \item $G$ is an initial RMG,
  \item $\mathcal{R}$ is a DPO rule set,
  \item $C$ is a coarse-graining / decoding from RMG states to "physical" macrostates,
...
\begin{enumerate}
  \item (Fidelity) For relevant time/length scales, $C$ of RMG dynamics matches predictions of $\mathcal{T}$.
  ...
  \item (Sparsity) The RMG representation is not absurdly inflated relative to the physics it encodes.
\end{enumerate}
...
Step one: turn “the world” into an RMG state.
...
RMG recursion lets us stack these neatly: "the field" is itself a graph that
...
This is exactly the kind of recursive structure RMGs are built for: graphs of graphs of labels.

\paragraph{Observers as subgraphs.}

We won’t pretend observers are mystical. In RMG:
...
\subsubsection*{26.5  Worked Example: 1D Wave Equation as RMG}
...
This is literally a group action on the-labels-of-the-graph. In RMG:
...
We want a clean dictionary between physics lab talk and RMG ops.
...
\begin{itemize}
  \item A \textbf{physical observable} $O$ is a function on (coarse-grained) graphs:
    \[
      O : \text{RMG states} \to \mathbb{R}^k \text{ or some measurable space}.
    \]
...
- We can model "apparatus" not as magical projection operators, but as additional RMG subgraphs that implement $C$ via interaction.
...
Just because you can encode a theory as RMG + DPO doesn’t mean you did it well.
...
Once a physical theory is compiled into RMG + DPO:
...
\noindent
That’s what the next chapter, \emph{CΩSMOS: The Physical Universe as RMG},
...
to treat "the universe" as just one RMG instance running inside CΩMPUTER.