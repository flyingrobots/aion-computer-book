Here is the revised Chapter 18. I have incorporated the **Lagrangian of Computation**, the **Horizon of Optimizability**, the explicit **EAL connection**, and the **formal theorem** in the sidebar, while maintaining the prose improvements and diagrams from the previous draft.
...
\[
\mathcal{L} = \alpha \cdot \text{Cost}_{rewrite} + \beta \cdot \Delta_{memory} + \delta \cdot \text{Interference}(B)
\]
\end{definition}
...
This is a profound and complex operation, as it involves navigating the meta-space of all possible rule sets. The mathematics required to perform these "rulial updates" safely is the subject of \textbf{Differential Rulial Analysis}, which we will explore fully in \textbf{Chapter 21}. For now, understand that the ultimate optimization is not finding the best path through the universe, but designing a universe where the best path is trivial.
...
We call this Rulial Provenance and the Eternal Audit Log. That is the subject of Chapter 19—the final machine of Part IV.
```