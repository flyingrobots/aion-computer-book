...
\label{chapter-10-local-np-collapse}

\section{\texorpdfstring{\textbf{Why some "hard" problems suddenly flatten when the manifold cooperates.}}{10.1 --- Why some "hard" problems suddenly flatten when the manifold cooperates.}}
...
\section{\texorpdfstring{\textbf{NP is not a property of the problem. It is a property of the representation.}}{10.2 --- NP is not a property of the problem. It is a property of the representation.}}
...
The same logic holds in the \COMPUTER{} framework. \textbf{WARP geometry determines search shape}.
...
\section{\texorpdfstring{\textbf{Structured Manifolds Create Shortcuts}}{10.3 --- Structured Manifolds Create Shortcuts}}
...
\begin{tikzpicture}[scale=0.9]
    ...
    \end{scope}
\end{tikzpicture}
\caption{Visualizing Local Collapse: In a naive representation (left), the distance between Start and Target is vast. In a structured WARP graph (right), a DPO rule "folds" the space, making the Target adjacent to the Start.}
\end{figure}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{\textbf{Local NP Collapse Looks Like Surfing the Rulial Surface}}{10.4 --- Local NP Collapse Looks Like Surfing the Rulial Surface}}
...
Suddenly, the geometry shifts. The search space flattens out. The jagged peaks of complexity smooth into a gentle slope. Geodesics—optimal paths—appear where there were once only tangles. The Time Cone widens, offering you clear, unblocked futures.
...
\section{\texorpdfstring{\textbf{Why WARP Recursion Creates Collapse Zones}}{10.5 --- Why WARP Recursion Creates Collapse Zones}}

The secret weapon of the WARP graph model—the feature that distinguishes it from flat graphs or standard relational models—is recursion. \textbf{WARP recursion lets you solve subproblems at the right scale.}

In a flat system, every node is a peer, meaning every interaction must be checked against every other interaction. But in a WARP graph, you can rewrite inside a single node, rewrite the internal logic of an edge, or rewrite entire sub-WARP blobs as single units. You can lift computation up to a high-level abstraction or lower it into the details. You can flatten nested structures when they get in the way or reorient wormholes to bypass layers of hierarchy.
...
\section{\texorpdfstring{\textbf{DPO Rules as Search Constraints}}{10.6 --- DPO Rules as Search Constraints}}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[thick, ->] (2, 2) -- (1, 0);
    \draw[thick, ->] (2, 2) -- (3, 0);
    \filldraw[fill=red!20, draw=red!70] (-3, 0) circle (0.3);
    \node[font=\scriptsize] at (-3, 0) {$\times$};
    \filldraw[fill=red!20, draw=red!70] (-1, 0) circle (0.3);
    \node[font=\scriptsize] at (-1, 0) {$\times$};
    \filldraw[fill=red!20, draw=red!70] (1, 0) circle (0.3);
    \node[font=\scriptsize] at (1, 0) {$\times$};
    \filldraw[fill=green!20, draw=green!60, very thick] (3, 0) circle (0.34);
    \node[font=\scriptsize] at (3, 0) {Goal};
...
\section{\texorpdfstring{\textbf{Rulial Curvature and NP Behavior Are the Same Thing}}{10.7 --- Rulial Curvature and NP Behavior Are the Same Thing}}
...
You are literally surfing the manifold of complexity. If you inhabit a jagged area of Rulial Space, where constraints clash and invariants overlap, the wormholes will fail to connect. The Time Cone will shrink, dead ends will multiply, and the search will explode.

However, if you navigate to a smooth area of the manifold, where constraints align and invariants reinforce one another, the wormholes interlock perfectly. The Time Cone widens, worldlines cluster together, and the search collapses. NP is not an absolute constant of the universe. \textbf{NP is curvature.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{\textbf{The Practical Takeaway: Sometimes You Win Because the Universe is Kind}}{10.8 --- The Practical Takeaway: Sometimes You Win Because the Universe is Kind}}
...
\section{\texorpdfstring{\textbf{NP Collapse is Local, Not Global}}{10.9 --- NP Collapse is Local, Not Global}}
...
\section{\texorpdfstring{\textbf{Transition: From Collapse to Bundles}}{10.10 --- Transition: From Collapse to Bundles}}
